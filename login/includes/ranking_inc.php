...
class Ranking {
	...
	function readR($a,$b){
		$query = "SELECT * FROM jumlah_alternatif_kriteria where id_kriteria='$b' and id_alternatif='$a'";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
	...
	function readMax($a){
		$query = "SELECT sum(skor_alt_kri) as mnr1 FROM jumlah_alternatif_kriteria where id_kriteria='$a'";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		return $stmt;
	}
	...
	function readHasil1($a){
		...
		$query = "SELECT sum(hasil_alt_kri) as bbn FROM jumlah_alternatif_kriteria WHERE id_alternatif='$a' LIMIT 0,1";
	...
	function normalisasi1(){
		...
		$query = "UPDATE jumlah_alternatif_kriteria
				SET
					hasil_alt_kri = :nn4
				WHERE
					id_alternatif = :ia
