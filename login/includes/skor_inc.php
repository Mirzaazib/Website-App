...
class Skor {
	...
	function insert3($a, $b, $c) {
		$query = "INSERT INTO jumlah_alternatif_kriteria VALUES('$a', '$b', $c, 0, 0)";
		$stmt = $this->conn->prepare($query);
	...
	function insert4($a, $b, $c) {
		$query = "UPDATE jumlah_alternatif_kriteria SET skor_alt_kri='$a' WHERE id_alternatif='$b' AND id_kriteria='$c'";
		$stmt = $this->conn->prepare($query);
	...
	function insert5($a, $b, $c) {
		$query = "UPDATE jumlah_alternatif_kriteria SET jumlah_alt_kri='$a' WHERE id_alternatif='$b' AND id_kriteria='$c'";
		$stmt = $this->conn->prepare($query);
...
//check
	...
	function readAll3($a, $b) {
		$query = "SELECT * FROM jumlah_alternatif_kriteria WHERE id_alternatif='$a' AND id_kriteria='$b' LIMIT 0,1";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
	...
	function readSum2($a, $b) {
		$query = "SELECT sum(hasil_analisa_alternatif) AS jumkr2 FROM {$this->table_name} WHERE alternatif_pertama='$a' AND id_kriteria='$b'";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this->nak = $row['jumkr2'];
	...
	function readSum3($a) {
		$query = "SELECT sum(skor_alt_kri) AS bbkr FROM jumlah_alternatif_kriteria WHERE id_kriteria='$a'";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this->bb = $row['bbkr'];
	...
	function readAvg($a,$b) {
		$query = "SELECT avg(hasil_analisa_alternatif) AS avgkr FROM {$this->table_name} WHERE alternatif_pertama = '$a' AND id_kriteria = '$b'";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this->hak = $row['avgkr'];
